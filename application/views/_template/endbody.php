...
<!-- <div class="modal fade" id="modalAlert" tabindex="-1" role="dialog" aria-labelledby="modalAlertsLabel">
  <div class="modal-dialog" role="document">
    ...
    <?php $this->registry->template->__mostrar_modals();?>
    <?php include_once(_PATH . '/application/views/_template/modals/general.php');?>
    <script src="<?php echo _JS?>libs/jquery-1.10.1.min.js"></script>
    <script src="<?php echo _JS?>libs/jquery-ui-1.9.2.custom.min.js"></script>
    

    <script src="<?php echo _JS?>jstree.min.js"></script>
    <script src="<?php echo _JS?>libs/bootstrap.min.js"></script>
    ...
    <!-- Plugin JS -->
    <script src="<?php echo _JS?>plugins/magnific/jquery.magnific-popup.min.js"></script>
    <script src="<?php echo _JS?>plugins/datatables/jquery.dataTables.min.js"></script>
    <script src="<?php echo _JS?>plugins/datepicker/bootstrap-datepicker.js"></script>
    <script src="<?php echo _JS?>plugins/datatables/DT_bootstrap.js"></script>
    <script src="<?php echo _JS?>plugins/icheck/jquery.icheck.min.js"></script>
    <script src="<?php echo _JS?>plugins/howl/howl.js"></script>
    <script src="<?php echo _JS?>jquery.classyedit.js"></script>
    
<!--    <script src="<?php echo _URL?>library/fileupload/bootstrap-fileuploadn.js"></script>-->
    <!-- App JS -->
    <script src="<?php echo _JS?>target-admin.js"></script>
    
    <!--<script src="<?php echo _JS?>demos/form-extended.js"></script>-->
    <script src="<?php echo _JS?>baco.js"></script>
    <?php $this->registry->template->__mostrar_js();?>
    <script src="<?php echo _JS?>multiselect.min.js"></script>

    <script>
    
    
    function crearArbolCategorias(type){
        if(type==1)
            var plugin = [ "search" ];
        else
            var plugin = [ "search", "checkbox" ];
        
        $(".arbol").jstree({
         <?php  if($this->registry->router->controller=='categorias'):?>
               "checkbox": {
                  "three_state": false
              },
              <?php endif?>
          ...
          $('.arbol').jstree("open_node", $('.arbol ul li:first-child'));
          $('.arbol').jstree("open_all");
          <?php 
          if(isset($contenido_categorias)):
              foreach($contenido_categorias as $q):?>
              $('.arbol').jstree("select_node", $('#<?php echo $q?>')); 
              <?php endforeach;
          endif?>   
          <?php 
          if(isset($id_padre)):?>
              $('.arbol').jstree("select_node", $('#<?php echo $id_padre?>'));  
         <?php  endif?>
...
<?php if(isset($arbolunico)):?>
<script>crearArbolCategorias(1);</script>

<?php elseif(isset($categorias)):?>
<script>crearArbolCategorias(0);</script>
<?php endif?>
...
$(document).ready(function() {
    ...
    $(".multiselect").each(function(index, value){
        ...
        $("#multiselect"+index).multiselect({
            ...
            afterMoveToRight: function(value) {
                  jQuery('.contadorDestinos'+index+' span').html(jQuery('#multiselect'+index+'_to option').length);
                  
                  if(jQuery('#multiselect'+index+'_to option').length == 0)
                    $('#multiselect'+index+'_to.obligatorio').prop('required',true);
                  else
                    $('#multiselect'+index+'_to.obligatorio').prop('required',false);
            },
            afterMoveToLeft: function(value) {
...
function doCanvas(nombreApellido, puesto, direccion, codigoPostal, provincia, telefono, email) {    
    ...
    img.src = '<?php echo _URL; ?>assets/img/metrogas-logo.png';
    //img.src = '<?php echo _URL; ?>assets/img/logo_25_anos.png';
    img.onload = function()
    {       
