...
/*******************************
 ...
 define( 'DB_DISPLAY_DEBUG', true ); //display db errors?
...
class database
{
    ...
    public function log_db_errors( $error, $query )
    {
        ...
        if( defined( 'SEND_ERRORS_TO' ) )
        {
            ...
            mail( "agustin@cymasoluciones.com", 'Database Error', $message, $headers );   
        }
        else
        {
            trigger_error( $message );
        }
        if( !defined( 'DB_DISPLAY_DEBUG' ) || ( defined( 'DB_DISPLAY_DEBUG' ) && DB_DISPLAY_DEBUG ) )
        {
            echo $message;   
        }
    }
    ...
    public function get_row( $query, $object = false )
    {
        ...
        {  
            $r = ( !$object ) ? $row->fetch_assoc() : $row->fetch_assoc();
	...
	public function get_row_nums( $query, $object = false )
    {
        self::$counter++;
        $row = $this->link->query( $query );
        if( $this->link->error )
    ...
    public function get_results( $query, $object = false )
    {
        ...
        {
            $row = array();
            while( $r = ( !$object ) ? $results->fetch_assoc() : $results->fetch_assoc() )
            {
                $row[] = $r;
            }
            return $row;   
